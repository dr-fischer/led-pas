...
# ╔═╡ 2a4ed042-f04a-11ec-389e-419be8d784d6
begin
	using JSServe
	Page()
end

# ╔═╡ 4ffdf74a-d344-425f-b83f-f502a8d9e4ed
begin
	using Makie
	using WGLMakie
...
# ╔═╡ 63217ce2-d56b-4d91-b0dd-632a7b89ecb3
md" # Front Panel"

# ╔═╡ 4594cc6d-790c-457a-8063-ef6ce73baf9f
md"""
$(@bind redraw PlutoUI.Button("Redraw Plot 📊"))
"""

# ╔═╡ 293ef2e4-5353-4bdf-8bde-0f189adfdc68
md"""
$(@bind dosweep PlutoUI.Button("Frequency Sweep 🧹"))


Sweep Start (Hz): $(@bind startf PlutoUI.NumberField(1000:10000; default=1370))
Sweep Stop (Hz): $(@bind stopf PlutoUI.NumberField(1000:10000; default=1380))
Set Frequency (Hz): $(@bind setf PlutoUI.NumberField(1000:10000; default=1370))
"""

# ╔═╡ c86df30b-d1ce-4121-b066-743c3fab6afb
@bind veg Radio(["Run", "Stop"], default = "Stop")

# ╔═╡ f21d36a0-f93c-4b5c-a0ff-d1850f04c094
md"# Internals"

# ╔═╡ ba702ceb-b881-4968-b1df-67ee817dafc8
md"## Packages"

# ╔═╡ 00c13bed-80ce-42a9-a8ce-970870867f31
md"## IO Setup"

# ╔═╡ d57683c4-3d9a-45d5-befa-1b9f2015e5e5
begin
	# list_ports()
...
# ╔═╡ 13118cd6-9f5b-47f3-9b19-b342b27be1eb
md"## Plot Setup"

# ╔═╡ b7aa69e2-2e2f-4ad6-803b-ee88dc082a72
md"## sweep()"

# ╔═╡ 064d663a-73de-4d93-a05c-d02d746420f7
md"## setFreq()"

# ╔═╡ 222b027a-958c-49f2-a76e-9d8440d3803c
function setFreq(portname::String, f::Int; echo = false, baud = 115200)
	LibSerialPort.open(portname, baud) do sp
		...
		if echo == true
			...
			readline(sp) # clear buffer by reading
		end
		close(sp)
	end
end

# ╔═╡ 71ca1b43-aadd-482d-a7c2-a942ca43475c
md" ## getMagAtF()"

# ╔═╡ 0e013f64-c345-4139-a74e-481f6a9c1ff6
function getMagAtF(fftdat, f::Int)
    return fftdat[findall(isapprox(f), fftdat[:, 1]), 2]
end

# ╔═╡ d72c9fa7-84b6-4849-988b-b6dd76add491
md" ## getFFT()"

# ╔═╡ bdd2ea41-aaa8-4d51-82c8-c92502059f00
...
# ╔═╡ 19002769-5855-4431-a584-872d3eb15e5e
begin
	# setup observables
...
# ╔═╡ b5ab2be5-f39a-41dc-80f2-6113408d2bf7
begin
	redraw
	...
	# # add data to figure
	barplot!(axs[1], x, y, color = :chartreuse4)
	lines!(axs[2], t, mag, linewidth = 2, color = :chartreuse4)
	# DataInspector(fig)
	# add axis labels
	axs[1].xlabel = "Frequency (Hz)"
	axs[1].ylabel = "Magnitude"
	axs[2].xlabel = "Elapsed Time (seconds)"
	axs[2].ylabel = "Magnitude at $(setf) Hz"
	xlims!(axs[1], 1300, 1400)
	ylims!(axs[1], 0, maximum(fftdat[:, 2])*1.2)
	fig
end

# ╔═╡ 8f86034d-7bf0-45bd-9f3c-399c986f3c3e
begin
	it = 1
	if veg == "Run"
		...
		@async begin
		    ...
		    # get new data and push to observables
			    global fftdat2 = getFFT(portname)
			    x[] = fftdat2[:, 1]
			    y[] = fftdat2[:, 2]
			    push!(t[], it)
			    t[] = t[]
			    push!(mag[], getMagAtF(fftdat2, setf)[1])
			    mag[] = mag[]
			
			    # autoscale axis limts
			    if it < bufferlength
			        xlims!(axs[2], t[][bufferlength-it], t[][bufferlength])
			        ylims!(axs[2], minimum(mag[][bufferlength-it+1:end])/1.2, maximum(mag[])*1.1)
			    else
...
# ╔═╡ 85786a1e-c815-4a62-966e-e02eaf901791
function sweep(portname::String, flow::Int, fhi::Int; echo = false, baud = 115200)
	l = fhi-flow+1
	x = ones(l)
	for n in 1:l
		f = n+flow
		setFreq(portname, f, echo = echo, baud = baud)
		fft = getFFT(portname, baud = baud)
		x[n] = getMagAtF(fft, f)[1]
	end
		return x
end

# ╔═╡ 96ea39a7-72b9-43b4-99e6-2ffa82f98174
begin
	dosweep
	mags = sweep(portname, startf, stopf) 
	sweepfig = Figure()
	Axis(sweepfig[1, 1], xlabel = "Frequency (Hz)", ylabel = "Magnitude", title = "Last Frequency Sweep at "*string(now()))
	sweepfig
	lines!(startf:stopf, mags, color = :green, linewidth = 2)
	sweepfig
end

# ╔═╡ 12d0a334-ac1a-4ed2-8a16-9f12a8e9b9d4
md" # Page Layout"

# ╔═╡ 604771ae-c4c2-4786-bca3-7d25347eeff7
html"""
<style>
	main {
		margin: 0 auto;
...
# ╔═╡ 7fc07be5-6b53-413f-a1d0-7ad50f5e8590
TableOfContents()
...
git-tree-sha1 = "de0e2006f45e7101e8ca310c81926452218fa515"
